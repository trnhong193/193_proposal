...
### C. Module Descriptions & Images (`type: "module_description"`, `diagram`)
- **Image Sizing**:
  - Images must be **prominent**. Avoid small thumbnails.
  - Target at least 40-50% of the slide area for the visual component.
  - Layout: Split screen (50% Text | 50% Image) is preferred over small floating images.
- **Source Handling**:
  - If `image_url` is present: Download and embed using Playwright for Google Drive links (handles authentication and redirects).
  - If `image_url` is missing or download fails: Generate a clean Placeholder image with the module title.
  - **Image Download**: 
...
- **Image URLs**: 
  - Support Google Drive sharing links (will be converted to direct download).
  - Empty or missing `image_url` will generate placeholder.
- **Mermaid Diagrams**: 
